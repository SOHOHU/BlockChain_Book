...
清算人希望偿还您的全部 $10,000 \text{ DAI}$ 债务，但协议的平仓系数是 $\mathbf{50\%}$。

* **单次最多可偿还的债务:** $10,000 \text{ DAI} \times 50\% = \mathbf{5,000 \text{ DAI}}$

#### B. 清算价差 (Liquidation Bonus) 的应用

清算人偿还了 $5,000 \text{ DAI}$ 的债务，作为回报，他将获得价值高于 $5,000 \text{ DAI}$ 的 $\text{ETH}$ 抵押品。

* **清算人获得抵押品的价值:** $5,000 \text{ USD} \times (1 + \text{LB})$
    $$
    5,000 \text{ USD} \times (1 + 5\%) = 5,000 \text{ USD} \times 1.05 = \mathbf{5,250 \text{ USD}}
    $$
...
1.  **第一步：初始借款**
    * 抵押 $10 \text{ ETH}$，借出 $10,000 \text{ DAI}$（$10,000 \text{ USD}$）。
...
4.  **第四步：继续借款（可选）**
    * 利用这 $5 \text{ ETH}$ 的新抵押品，您可以**再次借出**更多的 $\text{DAI}$，并重复步骤 2 和 3，直到您的健康因子接近 $1$。
